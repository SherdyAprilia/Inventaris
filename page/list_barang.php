...
<form class="navbar-form navbar-right" role="search" method="gets">
...
<table class="table table-bordered table-striped">
    ...
    <tbody>
      ...
      $sql = "SELECT * FROM invetaris LEFT JOIN ruang ON ruang.id_ruang = invetaris.id_ruang WHERE 1=1 $q_cari LIMIT $mulai, $pembagian";
      $query = mysqli_query($koneksi, $sql);
      $cek = mysqli_num_rows($query);
      //echo $cek;
      $sql_total = "SELECT * FROM invetaris";
      ...
      if($cek > 0){
        ...
        while($data = mysqli_fetch_array($query)){
          $tgl = $data['tanggal_regiser'];
        ?>
        <tr>
          <td><?= $no++ ?></td>
          <td><?= $data['kode_invetaris']?></td>
          <td><?= $data['nama']?></td>
          <td><?= $data['kondisi']?></td>
          <td><?= $data['jumlah']?></td>
